...
/** 串口BUF大小 */
#define USART_BUF_SIZE          (2048ul)  // 发送/接收缓存
...
typedef struct
{
    ...
    uint16_t  end;                                   /*!< 循环队列尾哨兵 */
    
    uint16_t  head;                                  /*!< 循环队列首哨兵 */
...
static void RecvDataHandler(USART_DriveHandleType *pHandle, uint8_t data)
{
    StartRecvByteTimer(pHandle, 100);
...
static int AddByteToBuffer(UBufferCtrl_t* pCtrl, uint8_t *pBuf, uint8_t data)
{
    if (0x00 == ((pCtrl->state) & 0x02) && pBuf != NULL)                      /* 允许向缓冲区写入数据 */
    {
        pBuf[(pCtrl->end)++] = data;
